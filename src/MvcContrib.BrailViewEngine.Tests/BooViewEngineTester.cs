...
namespace MvcContrib.UnitTests.BrailViewEngine
{
	...
	public class BooViewEngineTester
	{
		...
		private static readonly string VIEW_ROOT_DIRECTORY = @"Views";
...
//			_viewContext = new ViewContext(controllerContext, null, new ViewDataDictionary(), null);
		...
		{
			_mocks.ReplayAll();
			_viewEngine.Options.AssembliesToReference.Add(GetType().Assembly);
			_viewEngine.Options.BaseType = typeof(TestBrailBase).FullName;
